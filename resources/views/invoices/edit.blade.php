            ...
            itemProps.forEach( (i) => {
                ...
                $(`.${i}`).each(function() {
                    ...
                    if($(this).hasClass('itemQuantity')) {
                        let availableStocks = $(this).closest('tr').find('.itemStock').val();
                        let enteredQty = $(this).attr('data-qty') ?? 0;
                        let totalStocks = Math.floor(Number(enteredQty) + Number(availableStocks));
                        
                        if (Number($(this).val()) > Number(totalStocks)) {
